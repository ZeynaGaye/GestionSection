...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($validationErrors)) {
        if (rechercher_par_login($login) == 0 & rechercher_par_email($email) == 0) {
            $requete = $pdo->prepare("INSERT INTO etudiant(nom,prenom,login,email,NumCarte,idClasse,pwd,etat) 
                                        VALUES(:pnom,:pprenom,:plogin,:pemail,:pNumCarte,:pniveau,:ppwd,:petat)");
...
<div class="container col-md-6 col-md-offset-3">
    <h1 class="text-center"> Création d'un nouveau compte Etudiant</h1>
   ...
   <input type="text"
        ...
        <input type="text"
                required="required"
                name="NumCarte"
                placeholder="Taper votre Numéro de carte  "
                autocomplete="off"
                class="form-control">
       </div>
       <div class="input-container">
